...
#cargar el workspace after_conversion.RData que puede encontrar clonando el repositorio:
# https://github.com/Rumoa/preproc_classif

source("conversion.R")

#######################EN CASO DE USAR EL REPOSITORIO########################
setwd("files") #se guardarán y leerán archivos desde /files/
...
ggplot(data = grid.results.test, aes(x = C, y = F1)) + geom_point(stat ="identity", colour = "blue") + 
  labs( title = "Score F1 conjunto de test para J48 \ncon distintos valores de C",
